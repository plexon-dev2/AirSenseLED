/**
 * @file RTC.h
 * @brief General-purpose DS1307 RTC Driver
 *
 * Platform-independent DS1307 driver.
 * To port to your platform, implement the HAL functions at the
 * bottom of RTC.cpp:
 *   - RTC_HAL_I2C_Write()
 *   - RTC_HAL_I2C_Read()
 *   - RTC_HAL_DelayMs()
 ...
 * @return RTC_OK on success, RTC_ERR_I2C on failure
 */
 ...
 * @return true if running, false if halted (CH bit set)
 */
...
/**
 * @brief Set date and time using struct
 ...
 * @param p_buf   Buffer to write into (minimum 20 bytes)
...
/**
 * @brief Validate a date/time struct for correctness
...
/**
 * @brief Check if RTC backup battery is low
 * @note  DS1307 does not have a dedicated low-battery flag.
 *        This function checks if the oscillator has stopped (CH bit),
 *        which typically indicates power loss / dead battery.
 * @return true if battery may be low (oscillator was halted)
 */
bool RTC_IsBatteryLow(void);

/**
 * @brief Reset RTC to 2000-01-01 00:00:00 and restart oscillator
 * @return RTC_OK on success, error code on failure
 */
...
/*==============================================================================
 *                          HAL INTERFACE
 *  Implement these three functions in RTC.cpp for your platform.
 *============================================================================*/

/**
 * @brief Initialize I2C bus for RTC — call once in setup() before RTC__Init()
 * @note  Arduino platform only. Uses pins from RTC_Cfg.h.
 */
