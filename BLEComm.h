...
/**
 * @brief BLE operation status — mirrors WiFiComm_Status pattern
 */
typedef enum
{
    BLECOMM_STATUS_OK      =  0, /**< Operation successful           */
    BLECOMM_STATUS_ERROR   = -1, /**< Operation failed               */
    BLECOMM_STATUS_BUSY    = -2, /**< Module busy (connect pending)  */
    BLECOMM_STATUS_NO_CONN = -3  /**< No active BLE connection       */
} BLEComm_Status;
...
typedef enum
{
    ...
    BLECOMM_SERVER_IDENTIFIED   = 2U  /**< PyScript identified — data ON  */
} BLEComm_ServerState;
...
/**
 * @brief BLE communication periodic handler
 * @details Polls connect task completion, handles reconnect logic.
 *          Call every BLECOMM_HANDLER_INTERVAL_MS (100ms) from scheduler.
 * @note    BLE stack itself is event-driven via callbacks — this handler
 *          only manages state transitions not covered by callbacks.
 */
...
/**
 * @brief Send sensor data via BLE server NOTIFY
 * @details SERVER mode — pushes JSON to PyScript/phone connected to ESP32.
 *          Safe to call always — does nothing if not connected or not identified.
 *          Generic — any null-terminated JSON string accepted.
 * @param[in] json Null-terminated JSON string to send
 */
void BLEComm__SendSensorData(const char *json);

/**
 * @brief Send sensor data via BLE client WRITE
 * @details CLIENT mode — pushes JSON when ESP32 is connected TO PyScript.
 *          Safe to call always — does nothing if not in client mode.
 *          Generic — any null-terminated JSON string accepted.
 * @param[in] json Null-terminated JSON string to send
 */
 ...
 *          Results available via BLEComm__GetDeviceCount() and
 *          BLEComm__GetDeviceName() / BLEComm__GetDeviceRSSI().
 */
...
/**
 * @brief Connect to scanned device by index
 * @details Non-blocking — spawns background FreeRTOS task.
 *          Use BLEComm__IsDeviceConnecting() to poll progress.
...
/**
 * @brief Check if BLE server is actively sending data to identified client
 * @return true if PyScript identified and NOTIFY active
 */
bool BLEComm__IsSendingData(void);

/**
 * @brief Check if BLE client mode is active and sending data to PyScript
 * @return true if connected to PyScript as client and characteristic ready
 */
 ...
 * @return Device count (0 to BLECOMM_MAX_DEVICES)
 */
 ...
 * @param[in] index Device index (0 to GetDeviceCount()-1)
 * @return Pointer to device address string, or NULL if index invalid
 */
 ...
 * @param[in] index Device index (0 to GetDeviceCount()-1)
 * @return true if connect is in progress for this device
 */
bool BLEComm__IsDeviceConnecting(uint8_t index);

/**
 * @brief Get name of device connected to server (phone/PyScript → ESP32)
 * @return Pointer to server client name string
 */
const char *BLEComm__GetServerClientName(void);

/**
 * @brief Check if any device is connected to BLE server
 * @return true if server has a client
 */
bool BLEComm__IsServerClientConnected(void);

/**
 * @brief Check if PyScript is identified and receiving sensor data
 * @return true if PyScript identified via WRITE characteristic
 */
bool BLEComm__IsPyScriptConnected(void);

/**
 * @brief Get name of device connected as client (ESP32 → remote)
 * @return Pointer to connected device name string
 */
const char *BLEComm__GetConnectedDeviceName(void);

/*──────────────────────────────────────────────────────────────────────────────
 *  Dirty Flag API  (cross-task safe display signal)
 *  BLE callbacks set the flag; loop() polls and clears it before redrawing.
 *────────────────────────────────────────────────────────────────────────────*/

/**
 * @brief Check if BLE state changed and display needs update
 * @return true if status bar redraw is needed
 */
...
/**
 * @brief Set the dirty flag — called from BLE callbacks to signal redraw needed
 */
void BLEComm__SetStatusDirty(void);

/**
 * @brief Suppress or allow BLEComm__Handler auto-restart of advertising
 * @details Call with true before App_BLEStop() to prevent handler restarting adv.
 *          Call with false before App_BLEStartAdvertising() to re-enable.
 * @param[in] suppress true = suppress, false = allow
 */
