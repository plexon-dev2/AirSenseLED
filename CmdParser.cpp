 ...
 *          (LEDHMI, Modbus), corrupting Modbus frames and causing CRC FAIL.
 *          Use CMDPARSER_DEBUG_PRINTF from CmdParser_cfg.h instead.
 */
...
/**
 * @brief No-data watchdog: re-send ZPHS01C startup command after this many ms
 *        of silence. Handles mid-session sensor swap without a reboot.
 *        Well above the normal 1s frame interval -- normal operation never fires this.
 */
#define CMDPARSER_NO_DATA_RESEND_MS     (5000U)
...
/**
 * @brief Timestamp of last ZPHS01C startup command re-send (no-data watchdog).
 *        File scope so CmdParser__Reset() can clear it.
 */
static uint32_t s_lastResendMs = 0U;
...
static bool                  VerifyChecksum_ZPHS01B(const uint8_t *frame);
static bool                  VerifyChecksum_ZPHS01C(const uint8_t *frame);
...
static CmdParser_ErrorType_t ParseMessage_ZPHS01C(void)
{
    ...
    if (!VerifyChecksum_ZPHS01C(rx_buffer))
...
static CmdParser_ErrorType_t ParseMessage(void)
{
    ...
    if (!VerifyChecksum_ZPHS01B(rx_buffer))
        return CMDPARSER_ERROR_CHECKSUM_FAIL;
#endif
...
void CmdParser__Init(void)
{
    ...
    if (data_mutex == NULL)
    {
        ...
        if (data_mutex == NULL)
        {
            /* Heap exhausted -- cannot continue safely */
            Serial.println("[CMDPARSER] FATAL: mutex alloc failed");
    ...
    if ((saved == SENSOR_9PARAM) || (saved == SENSOR_5PARAM))
    {
        ...
        /*
         * Always send the ZPHS01C startup command on every boot regardless
         * of NVS saved type. This supports hot-swap: if the user replaces
         * the sensor while the device is off, the new sensor gets the command
         * it needs on the very next boot.
         *
         * Safe for ZPHS01B: it uses a completely different protocol (start
         * byte 0xFF, 26-byte frames) and silently ignores this command,
         * continuing to auto-stream normally.
         */
        CmdParser_ZPHS01C_SendStartupCommands();
    }
    else
...
void CmdParser__Handler(void)
{
    ...
    /*
     * No-data watchdog: if no valid frame received for CMDPARSER_NO_DATA_RESEND_MS,
     * re-send the ZPHS01C startup command. Handles mid-session sensor swap.
     * CMDPARSER_NO_DATA_RESEND_MS and s_lastResendMs are file-scope (not local).
     */
    if ((last_valid_data_time > 0U) &&
        ((current_time - last_valid_data_time) >= CMDPARSER_NO_DATA_RESEND_MS) &&
        ((current_time - s_lastResendMs)        >= CMDPARSER_NO_DATA_RESEND_MS))
    {
        s_lastResendMs = current_time;
        CmdParser_ZPHS01C_SendStartupCommands();
#if CMDPARSER_DEBUG_ENABLE
        Serial.println("[CMDPARSER] No data for 5s — re-sent startup command (sensor swap?)");
#endif
    }
     ...
     */
    /* uint16_t to safely accommodate future frame length increases beyond 255 */
    uint16_t active_frame_len  = (s_detected_sensor_type == SENSOR_5PARAM) ?
                                  (uint16_t)ZPHS01C_FRAME_LENGTH :
                                  (uint16_t)ZPHS01B_FRAME_LENGTH;
...
CmdParser_StatusType_t CmdParser__Reset(void)
{
    ...
    if (xSemaphoreTake(data_mutex, pdMS_TO_TICKS(100)) == pdTRUE)
    {
        ...
        last_valid_data_time = 0U;
        new_data_available   = false;
        s_lastResendMs       = 0U;   /* reset no-data watchdog timestamp */
...
static bool IsDataFresh(void)
{
    if (last_valid_data_time == 0U) { return false; }
    /* Unsigned subtraction is always safe for overflow -- wraps correctly */
    return ((millis() - last_valid_data_time) < DATA_FRESHNESS_TIMEOUT_MS);
}

/**
 * @brief Verify ZPHS01B frame checksum.
 * @details Checksum = (~sum(byte1..byte24) + 1) & 0xFF.
 *          Only valid for the 26-byte ZPHS01B frame.
 * @param[in] frame Pointer to 26-byte frame buffer.
...
static bool VerifyChecksum_ZPHS01B(const uint8_t *frame)
{
    ...
    if (!IsValidPointer(frame)) { return false; }
    ...
    return (((~sum + 1U) & 0xFFU) == frame[ZPHS01B_FRAME_LENGTH - 1U]);
}

/**
 * @brief Verify ZPHS01C frame checksum.
 * @details Checksum = (~sum(byte0..byte12) + 1) & 0xFF.
 *          Only valid for the 14-byte ZPHS01C frame.
 * @param[in] frame Pointer to 14-byte frame buffer.
 * @return true if checksum matches.
 */
static bool VerifyChecksum_ZPHS01C(const uint8_t *frame)
{
    ...
    for (i = 0U; i < (ZPHS01C_FRAME_LENGTH - 1U); i++)
    {
        sum += frame[i];
    }

    return (((~sum + 1U) & 0xFFU) == frame[ZPHS01C_FRAME_LENGTH - 1U]);
}